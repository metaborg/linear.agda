...
module M where
  ...
  frame sep c env s =
    ...
    in bind
      (λ px s' → λ where nil s'' → pure $ E₂ ×⟨ subst (_ ⊎ _ ≣_) (⊎-identity⁻ʳ s'') s' ⟩ px)
      c eq₂ E₁ (⊎-comm eq₁)

  asks : ∀ {P} → ∀[ (Env Γ ─✴ P) ⇒ M Γ ε P ]
  asks f env σ =
    let px = f env σ
    in pure (nil ×⟨ ⊎-identityˡ ⟩ px)

  prepend : ∀[ Env Γ₁ ⇒ M Γ₂ (Γ₁ ∙ Γ₂) Emp ]
  prepend env₁ env₂ s = pure $ env-∙ (env₁ ×⟨ s ⟩ env₂) ×⟨ ⊎-identityʳ ⟩ refl

  append : ∀[ Env Γ₁ ⇒ M Γ₂ (Γ₂ ∙ Γ₁) Emp ]
  append env₁ env₂ s = pure $ env-∙ (env₂ ×⟨ ⊎-comm s ⟩ env₁) ×⟨ ⊎-identityʳ ⟩ refl
...
module _ where
  ...
  eval′ (app (f ×⟨ Γ≺ ⟩ e)) =
    frame Γ≺ (eval′ f) ⟪ ⊎-identityʳ ⟫= λ where
  ...
  eval′ (send (e ×⟨ Γ≺ ⟩ ch)) =
    frame Γ≺ (eval′ e) ⟪ ⊎-identityˡ ⟫= λ where
      v σ₁ → eval′ ch ⟪ ⊎-comm σ₁ ⟫= λ where
        (chan φ) σ₂ → lift-f (Free.send! (φ ×⟨ ⊎-comm σ₂ ⟩ v)) ⟪ ⊎-identityˡ ⟫= λ where
          v σ₃ → subst (M _ _ _) (⊎-identity⁻ˡ σ₃) (return (chan v))

  eval′ (recv e) =
    eval′ e ⟪ ⊎-identityˡ ⟫= λ where
  ...
  eval e =
    Free.f-map
      (λ where (nil ×⟨ σ₁ ⟩ qx) σ₂ → subst (Val _) (trans (⊎-identity⁻ˡ σ₁) (⊎-identity⁻ˡ σ₂)) qx)
      (eval′ e nil ⊎-identityˡ) ⊎-identityˡ
